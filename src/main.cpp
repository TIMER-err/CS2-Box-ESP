...
int main()
{
    ...
    printf("[*] Press F4 to toggle TriggerBot.\n");
    printf("[*] Press F5 to toggle AimBot.\n");
    if (!load_offsets())
    {
        std::cerr << "[!] Failed to load offsets, exiting..." << std::endl;
        system("pause");
        return 1;
    }
    HWND hwnd = FindWindowW(NULL, L"Counter-Strike 2");
    if (!hwnd)
        hwnd = FindWindowW(NULL, L"反恐精英：全球攻势");
    if (!hwnd)
    {
    ...
    while (true)
    {
        ...
        localPlayerExists = get_local_entity(hProc, base, localPlayer);
        if (!localPlayerExists)
            continue;
        ents = get_entities(hProc, base);
        Aimbot::Run(hProc, base, localPlayer, ents, width, height, matrix);
        for (auto &ent : ents)
        {
            if (ent.hp <= 0)
                continue;
            if (!ent.project(matrix, width, height))
                continue;

            TriggerBot::Run(localPlayer, ent, width, height);
            ...
            COLORREF col = (ent.team != localPlayer.team) ? RGB(255, 0, 0) : RGB(0, 128, 255);
            ...
            COLORREF hpcol = hp_ratio > 0.66f ? RGB(0, 255, 0) : hp_ratio > 0.33f ? RGB(255, 255, 0)
                                                                                  : RGB(255, 0, 0);
            ...
            boxH = static_cast<float>(ent.feet2d.y - ent.head2d.y) * 0.2f;
            boxW = boxH;
            x = ent.head2d.x - boxW / 2.0f;
            y = ent.head2d.y - boxH * 0.3f;
